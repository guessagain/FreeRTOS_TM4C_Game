...
void Nokia5110_OutCharU(unsigned char data){
  int i,num;
  lcdwrite(DATA, 0x00);                 // blank vertical line padding
  for(i=0; i<5; i=i+1){
		num = (ASCII[data - 0x20][i] & 0x0F) <<4;
    lcdwrite(DATA, num);
  }
  lcdwrite(DATA, 0x00);                 // blank vertical line padding
}
void Nokia5110_OutCharD(unsigned char data){
  int i,num;
  lcdwrite(DATA, 0x00);                 // blank vertical line padding
  for(i=0; i<5; i=i+1){
		num = (ASCII[data - 0x20][i] & 0xF0) >>4;
    lcdwrite(DATA, num);
  }
  lcdwrite(DATA, 0x00);                 // blank vertical line padding
}
...
void Nokia5110_OutDec(unsigned short n){
  if(n < 10){
		Nokia5110_OutChar('0');
    Nokia5110_OutChar(n+'0'); /* n is between 0 and 9 */
  } else if(n<100){
...
//********Nokia5110_Setpix*****************
//Set cursoe at given pixel position.
//X-axis ranges from 0 t0 83
//Y-axis ranges from 0 t0 5.
